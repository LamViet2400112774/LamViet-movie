...
// 👉 FIX URL ẢNH OPHIM
// ===========================
function getOphimImage(url) {
    if (!url) return "/no-image.jpg";

    // Nếu API trả /uploads/... → gắn domain CDN mới
    if (url.startsWith("/uploads")) {
        return "https://img.ophim.live" + url;
    }

    // Thay domain cũ đã chết
    return url
        .replace("img.ophim9.cc", "img.ophim.live")
        .replace("img.ophim10.cc", "img.ophim.live")
        .replace("ophim9.cc", "ophim.live");
}
...
const MovieCard = ({ movie }) => {
    ...
    const imgUrl = getOphimImage(movie.thumb_url || movie.poster_url);
                ...
                <div className="overflow-hidden rounded-lg lg:rounded-xl relative">
                    <div className="relative aspect-[9/13.5] group-hover:blur-sm group-hover:brightness-50
                        ...
                        <Image
                            ...
                            loading="lazy"
                            quality={40}
                            unoptimized   // 🔥 BẮT BUỘC để tránh lỗi Next
                        />
